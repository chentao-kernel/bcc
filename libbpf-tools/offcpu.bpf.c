/*
 * Create: Mon Jan 12 11:28:32 2026
 */
...
int sched_switch_hook(struct pt_regs *ctx)
{
        ...
        delta = (curr_ts - ep->target.offcpu_ns) / 1000;
        if ((delta >= args->min_offcpu_us) && (delta <= args->max_offcpu_us) &&
                /*
                * if offcpu_ns = 0, means we do not trace wake and sched_switch
                * two step completely, so we ignore it.
                */
            ep->target.offcpu_ns != 0) {
                event = bpf_map_lookup_elem(&event_heap_map,
                                                 &args_map_id);
                if (!event)
                        return 0;
                ...
                runq_dur = (cur_task_run_delay_ns - ep->target.run_delay_ns) /
                           1000;
                event->target = ep->target;
                event->target.kern_stack_id =
                        bpf_get_stackid(ctx, &stack_map, BPF_F_FAST_STACK_CMP);
                event->target.user_stack_id = bpf_get_stackid(
                        ctx, &stack_map,
                        BPF_F_USER_STACK | BPF_F_FAST_STACK_CMP);
                bpf_get_current_comm(event->target.comm, TASK_COMM_LEN);
                event->waker = ep->waker;
                event->ts_ns = curr_ts;
                event->dur_us = delta;
                event->rq_dur_us = runq_dur;

                /* output task occupy cpu */
                if (runq_dur > args->rq_dur_us && args->rq_dur_us != 0) {
                        sched_cache_dump(ctx, cpu_id);
                        event->is_sched_cache_dump = 1;
                        event->cpu = cpu_id;
                }
                /* output event */
